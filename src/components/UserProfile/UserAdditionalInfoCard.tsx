...
export default function UserAdditionalInfoCard() {
  ...
  const [formData, setFormData] = useState({
    ...
    portfolio: "ridamihi.netlify.app"
  });
                  ...
                  <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                    <div>
                      ...
                      <Input
                        type="text"
                        name="certifications"
                        value={formData.certifications.map(cert => cert.name).join(", ")}
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          certifications: e.target.value.split(",").map(cert => ({
                            name: cert.trim(),
                            image: `/certifications/${cert.trim().toLowerCase().replace(/\s+/g, '-')}.png`,
