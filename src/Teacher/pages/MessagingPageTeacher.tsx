...
import AppointmentModal from '../components/Messaging/AppointmentModal';
import { Message, Conversation } from '../components/Messaging/types';

const RECIPIENT_TYPES = [
  { id: 'rida', name: 'Rida Mihi', avatar: 'user' },
  { id: 'sanae', name: 'Sanae Elmahi', avatar: 'user' },
  { id: 'achraf', name: 'Achraf Abattouy', avatar: 'user' },
  { id: 'sadik', name: 'Mohamed-Sadik Derbi', avatar: 'user' }
];

export default function MessagingPageTeacher() {
  const [conversations, setConversations] = useState<Conversation[]>([{
    id: '1',
    participants: [
      { id: 'currentUserId', name: 'Moi', avatar: 'user', isOnline: true },
      { id: 'rida', name: 'Rida Mihi', avatar: 'user', isOnline: true }
    ],
    messages: [
      {
        id: '1',
  ...
  const handleNewMessage = (recipientId: string, content: string, attachments?: string[]) => {
    ...
    setConversations(prevConversations => {
      ...
      const newConversation: Conversation = {
        ...
        participants: [
          ...
          {
            id: recipientId,
            name: RECIPIENT_TYPES.find(r => r.id === recipientId)?.name || 'Nouveau contact',
    ...
    if (sendMessage) {
      sendMessage({
        type: 'new_message',
        data: { conversationId: Date.now().toString(), message: newMessage }
      });
    }
  };
  ...
  const handleAppointmentRequest = (date: Date, time: string, notes: string) => {
    ...
    sendMessage({
      ...
      data: { conversationId: selectedConversation.id, message: appointmentMessage }
    });
    ...
    <div className="flex h-[calc(100vh-4rem)] flex-col md:flex-row">
      <div className={`w-full md:w-80 lg:w-96 ${selectedConversation ? 'hidden md:block' : 'block'} border-r border-gray-200 dark:border-gray-700`}>
        ...
        <div className="flex h-14 items-center space-x-2 border-b border-gray-200 px-4 dark:border-gray-700">
          <button onClick={() => setFilter('all')} className={`rounded-full px-3 py-1 text-sm ${filter === 'all' ? 'bg-primary text-white' : 'bg-transparent text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700'}`}>Tous</button>
          <button onClick={() => setFilter('unread')} className={`rounded-full px-3 py-1 text-sm ${filter === 'unread' ? 'bg-primary text-white' : 'bg-transparent text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700'}`}>Non lus</button>
          <button onClick={() => setFilter('archived')} className={`rounded-full px-3 py-1 text-sm ${filter === 'archived' ? 'bg-primary text-white' : 'bg-transparent text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700'}`}>Archivés</button>
        </div>
